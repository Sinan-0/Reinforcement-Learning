...
##see https://github.com/hermesdt/reinforcement-learning/blob/master/a2c/cartpole_a2c_online.ipynb
...
class OneStep_A2CAgent():
    ...
    def update(self, dist, action, next_state, reward, done, critic_output):
        
        #advantage = Qvals - values
        advantage = reward + (1-done)*self.gamma*self.critic(t(next_state)) - critic_output
...
def run_a2c(env, agent, nb_eps, max_steps, verbose = True):
    ...
    for episode in range(nb_eps):
        ...
        while (not done) and (nb_step < max_steps):
            ...
            agent.update(dist, action, next_obs, reward, done, critic_output)
